...
func CalcWidth(maxLineLen, paddingWidth int) int {
...
func CalcHeight(lineCount, paddingHeight int, fontSize float64, noWindowAccessBar bool) int {
...
func NewImage(src io.Reader, face font.Face, fontSize float64, style, backgroundColor string, paddingHeight, paddingWidth int, noWindowAccessBar, noLineNum bool) (*Panel, error) {
	...
	width := CalcWidth(font.MeasureString(face, " ").Ceil()*(ret+1), paddingWidth)
	height := CalcHeight(ln, paddingHeight, fontSize, noWindowAccessBar)
	...
	p.fontFace = face
	p.fontSize = fontSize
	p.paddingHeight = paddingHeight
	p.paddingWidth = paddingWidth
...
func (p *Panel) drawWindowControlPanel(w, h int) {
	...
	wc.paddingHeight = p.paddingHeight
	wc.paddingWidth = p.paddingWidth
